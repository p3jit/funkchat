...
function LeftSingleChat({value , lastMessage}) {
  const {dispatch , setMessages} = useContext(ChatContext);
  ...
  return (
    <div className='flex hover:bg-zinc-700 bg- p-3 rounded-lg cursor-pointer' onClick={handleClick}>
      ...
      <div className='pl-4 pt-2 flex flex-col justify-center relative bottom-2'>
        ...
        {lastMessage && <h1 className='truncate text-ellipsis w-40 text-sm text-zinc-300'>{lastMessage}</h1>}
      </div>
    </div>
  )
}
